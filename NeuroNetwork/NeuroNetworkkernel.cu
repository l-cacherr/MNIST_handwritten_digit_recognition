...
int main()
{
	cout << "HighRate Micro Modify\n";
	srand(time(NULL));
	vector<int> NeuroNetScale = { 784,16,16,10 };//
	//for (int i = 0; i < a.size(); i++) cout << a[i] << " ";
	NeuroNet NumRecogNet(NeuroNetScale, 1);//0
	NumRecogNet.ReadFromFile(".\\Net_ACR_0.944400_Round_3097400_2023_7_18_10_1_2.txt");//!!!
	...
	while (true)//for(int Idx = 0;Idx < 5000;Idx++)
	{
		...
		double BestACRate = BestACCount / (double)(TestImgCnt);
		//LearningRate = 0.000002 * (double)(TrainRound - ACBestRound) * 1.00 / ((BestACRate + 0.2) * (BestACRate + 0.2)) + 0.000001;
		LearningRate = 0.000002 * (double)(TrainRound - ACBestRound) * 1.00 / ((BestACRate + 0.2) * (BestACRate + 0.2)) + 0.000001;
		...
		if (BPCnt >= 100)
		{
			...
			if (((TestACCount - BestACCount) / (double)(TestImgCnt)) >= 0.0001)
			{
				BestACCount = TestACCount;
